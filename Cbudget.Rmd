...
write.csv(Daily.nGF, "/Users/marionnyberg/Google\ Drive/Micromet\ Lab/People/2019-Marion\ Nyberg/BB1-2/data/fluxNGF.csv")
...
write.csv(Daily, "/Users/marionnyberg/Google\ Drive/Micromet\ Lab/People/2019-Marion\ Nyberg/BB1-2/data/flux.csv")
...
BB2<-fread("/Users/marionnyberg/Google\ Drive/Micromet\ Lab/Projects/2019-Burns\ Bog\ 2/Flux-tower/met_data/met_merged/met_corrected_gapfilledBB2.csv") %>%
  ...
  mutate(TaS.30= mean(c(SOIL_TEMP_1_30CM, SOIL_TEMP_2_30CM, SOIL_TEMP_3_30CM))) %>%
  rename("WTH1" ="WTH") %>%
  filter(DATE >= '2020-01-01 01:00:00') 
BB2$Site <- "BB2"
BB2$DATE <- as.POSIXct(BB2$DATE, format="%Y-%m-%d %H:%M:%S", tz="UTC")
...
BB.daily <- BB.daily %>% rename(RH = RH_2M,
                   PARin = INCOMING_PAR,
                   SWin = SHORTWAVE_IN,
                   Ta = AIR_TEMP_2M, 
                   WTH. = WTH1, 
...
BB.daily <- BB.daily %>% 
  select(., TS.5, TS.10, RH, PARin, SWin, VPD, Ta, WTH., SWC, Precip, jday, Site, month) %>%
  group_by(jday, Site, month) %>%
...
### WTH
```{r}
#Linearly extrapolate to gap fill
WTH.lm <-BB.daily %>% 
  select(c(WTH._mean, Site, jday)) %>% 
           pivot_wider(names_from = Site, values_from = WTH._mean)
...
BB.daily$WTH._mean<- ifelse(BB.daily$Site == 'BB1', NA, BB.daily$WTH._mean)
BB.daily$WTH._mean[which(is.na(BB.daily$WTH._mean))] = WTH.lm$BB1
```

### Soil temperature
```{r}
#Linearly exptrapolate to gap fill
TS5.lm <-BB.daily %>% 
  select(c(TS.5_mean, Site, jday)) %>% 
           pivot_wider(names_from = Site, values_from = TS.5_mean)
...
BB.daily$TS.5_mean<- ifelse(BB.daily$Site == 'BB1', NA, BB.daily$TS.5_mean)
BB.daily$TS.5_mean[which(is.na(BB.daily$TS.5_mean))] = TS5.lm$BB1

BB.dailyGS <- BB.daily[(BB.daily$jday >= 92 & BB.daily$jday <= 274), ] 
BB.dailyNGS <-BB.daily[(BB.daily$jday < 92 | BB.daily$jday >=275), ] 

TS5.p <- ggplot(BB.daily, aes(jday, TS.5_mean)) + 
...
kruskal.test(TS.5_mean~Site, data = BB.daily)
kruskal.test(TS.5_mean~Site, data = BB.dailyGS)
kruskal.test(TS.5_mean~Site, data = BB.dailyNGS)


TS10.lm <-BB.daily %>% 
  select(c(TS.10_mean, Site, jday)) %>% 
           pivot_wider(names_from = Site, values_from = TS.10_mean)
TS10.lm.1 <- lm(BB1~BB2, TS10.lm)
summary(TS10.lm.1)
coef.TS10 <- 0.91690 
intercept.TS10 <- 1.73055
...
BB.daily$TS.10_mean<- ifelse(BB.daily$Site == 'BB1', NA, BB.daily$TS.10_mean)
BB.daily$TS.10_mean[which(is.na(BB.daily$TS.10_mean))] = TS10.lm$BB1

TS10.p <- ggplot(BB.daily, aes(jday, TS.10_mean)) + 
  geom_line(aes (color = Site)) +
...
kruskal.test(TS.10_mean ~ Site, data = BB.daily)
kruskal.test(TS.10_mean ~ Site, data = BB.dailyGS)
kruskal.test(TS.10_mean ~ Site, data = BB.dailyNGS)
```

### Air temperature
```{r}

(Ta_mean.p <- ggplot(BB.daily, aes(jday, Ta_mean)) + 
  geom_line(aes (color = Site)) +
...
kruskal.test(Ta_mean ~ Site, data = BB.daily)
...
```{r}
ggplotly(RH.p <- ggplot(BB.daily, aes(jday, RH_mean)) + 
...
kruskal.test(RH_mean~Site, data=BB.daily)
```
...
```{r}
write.csv(BB.daily, "/Users/marionnyberg/Google\ Drive/Micromet\ Lab/People/2019-Marion\ Nyberg/BB1-2/data/Full.csv")
```
### EC average data
...
write.csv(Fulldata, "BB_fulldata.csv")
write.csv(FulldataGS, "BB_fulldataGS.csv")
write.csv(FulldataNGS, "BB_fulldataNGS.csv")
...
BB1.CH4.lm1 <- lm(logCH4_mgC~TS.5_mean+WTH, data = BB1.model)
summary(BB1.CH4.lm1)

BB1.CH4.Ts <- lm(logCH4_mgC ~ TS.5_mean, data = BB1.model)
summary(BB1.CH4.Ts)
...
BB2.CH4.lm1 <- lm(logCH4_mgC~TS.5_mean+WTH, data = BB2.model)
summary(BB2.CH4.lm1)

BB2.CH4.TS5 <- lm(logCH4_mgC~TS.5_mean, data = BB2.model)
summary(BB2.CH4.TS5)
...
CH4.Ts <- lm(logCH4_mgC ~ Site * TS.5_mean, Fulldata)
summary(CH4.Ts)

# https://stats.stackexchange.com/questions/64927/how-do-i-plot-an-exponential-curve-in-r (below is following this)

mdl1 <- lm(logCH4_mgC ~ TS.5_mean, data = BB1.model)
mdl2 <- lm(logCH4_mgC ~  I(TS.5_mean^3), data = BB1.model)


summary(mdl1)
summary(mdl2) #higher R2

prd <- data.frame(TS.5_mean = seq(0, 20, by = 0.5))

result <- prd
#result$mdl1 <- predict(mdl1, newdata = prd)
result$mdl2 <- predict(mdl2, newdata = prd)

result <-  melt(result, id.vars = "TS.5_mean", variable.name = "model",
                value.name = "fitted")
ggplot(result, aes(x = TS.5_mean, y = fitted)) +
  theme_bw() +
  geom_point(data = BB1.model, aes(x = TS.5_mean, y = logCH4_mgC)) +
...
mdl3 <- lm(logCH4_mgC ~ TS.5_mean, data = BB2.model)
mdl4 <- lm(logCH4_mgC ~ I(TS.5_mean^3), data = BB2.model)

prd2 <- data.frame(TS.5_mean = seq(0, 20, by = 0.5))

result2 <- prd2
#result2$mdl3 <- predict(mdl3, newdata = prd)
result2$mdl4 <- predict(mdl4, newdata = prd2)

result2 <-  melt(result2, id.vars = "TS.5_mean", variable.name = "model",
                value.name = "fitted")
ggplot(result2, aes(x = TS.5_mean, y = fitted)) +
  theme_bw() +
  geom_point(data = BB2.model, aes(x = TS.5_mean, y = logCH4_mgC)) +
  geom_line(aes(colour = model), size = 1)
...
P1 <- Fulldata %>%
  filter(TS.5_mean > '0' & logCH4_mgC > '1') %>%
  ggplot(., aes(TS.5_mean, logCH4_mgC, color= Site)) +
  geom_point() +
      scale_colour_manual(values = c("#0072B2", "#D55E00"))+
  #geom_line(data = results, aes(TS.5_mean, fitted, color=model))+
  geom_line(color='#0072B2',data = result, aes(TS.5_mean, y=fitted))+
  geom_line(color='#D55E00',data = result2, aes(TS.5_mean, y=fitted))+  ##geom_smooth(method=)
...
linear_model<- lm(logCH4_mgC ~ TS.5_mean*Site, data = Fulldata)
summary(linear_model)
# fit the quadratic Model
quadratic_model <- lm(logCH4_mgC~ TS.5_mean + I(TS.5_mean^2) + WTH, data = BB1.model)
...
plot(BB1.model$TS.5_mean, BB1.model$logCH4_mgC,
     col  = "steelblue",
     pch = 20,
     xlab = "TS.5_mean",
     ylab = "logCH4_mgC",
...
order_id <- order(BB1.model$TS.5_mean)

lines(x = BB1.model$TS.5_mean[order_id], 
...
# fit the quadratic Model
quadratic_model <- lm(logCH4_mgC~ I(TS.5_mean^3) + WTH + I(WTH^3) , data = BB2.model)
summary(quadratic_model)
...
P1.1 <- FulldataGS %>%
  filter(TS.5_mean > '0') %>%
  ggplot(., aes(TS.5_mean, ch4mgC, color= Site)) +
  geom_point() +
  theme_bw()
P1.1
...
P1.3 <- BB1.model %>%
  filter(TS.5_mean > '0') %>%
  ggplot(., aes(TS.5_mean, ch4mgC, color= Period)) +
  geom_point() +
  theme_bw()
P1.3
