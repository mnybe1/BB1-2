...
Full <- read.csv("Full.csv") #All data (GS and NGS), below 2 new data frames separated into GS and NGS
FullGS <- Full[(Full$jday >= 92 & Full$jday <= 274), ] 
FullNGS <-Full[(Full$jday < 92 | Full$jday >=275), ] 
```
...
- NDVI - make sure gaps are filled
- Using 10 year average Environment Canada data, calculate deviattion from the mean for both sites
...
```{r Relative humidity}
(RH.p <- ggplot(Full, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), RH_mean)) + #as.Date adds the month names into the plot
...
(WTH.p <- ggplot(Full, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), WTH._mean)) + #as.Date adds the month names into the plot
...
(TS.5.p <- ggplot(Full, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), TS.5_mean)) + #as.Date adds the month names into the plot
...
(TS.10.p <- ggplot(Full, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), TS.10_mean)) + #as.Date adds the month names into the plot
  #geom_ribbon(aes(ymin = TS.5_min -1 , ymax = TS.5_max+1), fill = "light grey")+
    geom_line(aes (color=Site)) +
...
Flux.daily <- read.csv("/Users/marionnyberg/Google\ Drive/Micromet\ Lab/People/2019-Marion\ Nyberg/BB1-2/scripts/flux.csv")
FluxGS <- Flux.daily[(Flux.daily$jday >= 92 & Flux.daily$jday <= 274), ] 
FluxNGS <-Flux.daily[(Flux.daily$jday < 92 | Flux.daily$jday >=275), ] 
```
