...
Env <- read.csv("Full.csv") #All data (GS and NGS), below 2 new data frames separated into GS and NGS
EnvGS <- Env[(Env$jday >= 92 & Env$jday <= 274), ] 
EnvNGS <-Env[(Env$jday < 92 | Env$jday >=275), ] 
```
...
```{r Air temperature}
(Ta.p <- ggplot(Env, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), Ta_mean)) + #as.Date adds the month names into the plot
...
```{r Relative humidity}
(RH.p <- ggplot(Env, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), RH_mean)) + #as.Date adds the month names into the plot
...
(WTH.p <- ggplot(Env, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), WTH._mean)) + #as.Date adds the month names into the plot
...
(TS.5.p <- ggplot(Env, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), TS.5_mean)) + #as.Date adds the month names into the plot
...
(TS.10.p <- ggplot(Env, aes(x = as.Date(jday, origin = as.Date("2020-01-01")), TS.10_mean)) + #as.Date adds the month names into the plot
  #geom_ribbon(aes(ymin = TS.5_min -1 , ymax = TS.5_max+1), fill = "light grey")+
    geom_line(aes (color=Site)) +
...
Flux.daily <- read.csv("/Users/marionnyberg/Google\ Drive/Micromet\ Lab/People/2019-Marion\ Nyberg/BB1-2/data/flux.csv") 
...
Flux.Env <- right_join(Flux.daily, Env,  by = c("jday", "Site")) %>%
