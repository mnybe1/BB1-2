...
- NDVI (26/2)
  - make sure gaps are filled better than they are now
...
- Calculate radiative forcing - have gotten some values from K. Nugents code, although still trying to figure out how to make it predict over time... I think what I have now is the radiative forcing currently... I have a value of ~ -3 for total RF (CO2 and CH4)

- Compare my values to Nick and Brenda's to assess inter-annual variability.. (maybe)
...
- surface plots of CH4 response to temp / wth / gpp? 
...
Notes: 
- This needs lots of work to gap fill more accurately? 

```{r load NDVI data and gap fill, results=FALSE}
...
summary(ndvi.lm)
coef.ndvi <- 0.94298
intercept.ndvi <- -0.06652

NDVI <- NDVI %>% filter(DATE >= "2020-01-01")
NDVI$jday <- yday(NDVI$DATE)

jday <- c(1:366)
JDAY <- data.frame(jday)
NDVI.new<-merge(JDAY, NDVI,  by = 'jday', all.x = TRUE)
NDVI.new <- NDVI.new %>%
  select(c(jday, Site, NDVI))
...
```{r Reco and Temp effect , figures-side, fig.show="hold", out.width="50%"}
#NOTES:
...
Again, removing WTH from the model does slightly improve the model, suggesting that at both sites Reco is more strongly driven by soil temperature than WTH. At BB2, including WTH in the model sig. (P<0.05) improves it. BB2 gets significantly drier than BB1, which is why there is such a strong control of WTH on Reco.
...
#summary(model.3) #going to go with model 2
...
For BB2, inclding the interaction term does sig improve the models ability to predict CH4 fluxes. 
...
- from this data it seems like more vascular vegetation, driven by lower water tables, means that CH4 emissions are lower. This is surprising because I would've thought that more labile substrate from vascular veg --> more ch4 emissions, considering I dont think the SWC is limiting...
- I need to see how close the linear regression is though at each site, i.e. what is the fit like

```{r CH4 emissions and C sequestration}
(CH4.Cseq.p <-Flux.input_data %>% 
  ggplot(., aes(GPP_f, CH4mgC_f_RF, color = Site)) +
  geom_point()+
...
Need to look into this more as the relationships look different between sites. Including GPP in the model for CH4 prediction really improves the R2 value for BB1. 

```{r CH4 emissions and C sequestration BB1 and BB2 sep}
CH4.GPPBB1 <- lm(CH4mgC_RF_log~GPP_f * TS.5_mean * WTH._mean, BB1)
summary(CH4.GPPBB1)

(CH4.Cseq.p1 <- ggplot(BB1, aes(GPP_f, CH4mgC_RF_log)) +
  geom_point()+
   #geom_smooth(method = "lm", aes(colour = Site)) +
  #scale_colour_manual(values = c("#0072B2", "#D55E00")) +
  xlab(bquote('GPP (g C' ~m^-2~ day^-1*')'))+
...
CH4.GPPBB2 <- lm(CH4mgC_RF_log~GPP_f *TS.5_mean*WTH._mean, BB2)
summary(CH4.GPPBB2)

(CH4.Cseq.p1 <- ggplot(BB2, aes(GPP_f, CH4mgC_RF_log)) +
  geom_point()+
   #geom_smooth(method = "lm", aes(colour = Site)) +
  #scale_colour_manual(values = c("#0072B2", "#D55E00")) +
  xlab(bquote('GPP (g C' ~m^-2~ day^-1*')'))+
...
```{r GPP and NDVI}
(GPP.NDVI.p <-Flux.Env %>% 
  ggplot(., aes(NDVI, GPP_f, color = Site)) +
  geom_line()+
   #geom_smooth(method = "lm", aes(colour = Site)) +
  #scale_colour_manual(values = c("#0072B2", "#D55E00")) +
  #xlab(bquote('GPP (g C' ~m^-2~ day^-1*')'))+
