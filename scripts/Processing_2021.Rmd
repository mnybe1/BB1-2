...
- calculate Q10s. I had done this previously but I didn't include because there was such an important WTH effect...
- __HYSTERESIS__ - if focusing more on WTH as diff between the sites. - effect on Reco and CH4..
- surface plots of CH4 response to temp / wth / gpp ... working on
- __UNCERTAINTY__

There is obviously still a lot to do but here is what it's all looking like so far... 
...
(CH4.Cseq.p <-Flux.Env %>% 
  ggplot(., aes(GPP_f, CH4mgC_RF_log, color = Site)) +
  geom_point()+
...
At BB1, at a certain GPP value (~1), CH4 plateaus, suggesting there is some factor that is limiting CH4 production?
